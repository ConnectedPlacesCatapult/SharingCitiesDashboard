...
class AddNewLocationData(Resource):
    ...
    def __init__(self) -> None:
        ...
        self.reqparser.add_argument('timestamp', required=True, type=lambda x: datetime.strptime(x, '%d/%m/%y %H:%M'))
        ...
        self.reqparser.add_argument('signal_quality', required=True, type=float)
    ...
    def post(self) -> (str, HTTPStatus):
        ...
        if Tracker.get_by_tracker_id(args["tracker_id"]):
            ...
            loc_data.save()
            loc_data.commit()
            return dict(id=loc_data.id, tracker_id=loc_data.tracker_id), HTTPStatus.CREATED
        return dict(error="Tracker Not Found", tracker_id=args["tracker_id"]), HTTPStatus.NOT_FOUND
