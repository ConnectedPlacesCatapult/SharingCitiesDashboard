...
def test_create_user(test_client, admin_user):
    ...
    response_create = test_client.post('/admin/create_new_user',
                                       data=dict(email="new@gmail.com", fullname="New User", admin="True",
                                                 password="1234"), headers=header, follow_redirects=True)
...
def test_delete_user(test_client, admin_user, dummy_user):
    ...
    response_delete = test_client.delete('/admin/delete_user', data=dict(email=dummy_user.email), headers=header,
                                         follow_redirects=True)
...
def test_get_user(test_client, admin_user, dummy_user):
    ...
    response_get = test_client.post('/admin/get_user_by_email', data=dict(email=dummy_user.email), headers=header,
                                    follow_redirects=True)
...
def test_edit_user(test_client, admin_user, dummy_user):
    ...
    response_change = test_client.post('/admin/edit_user', data=dict(email=dummy_user.email, fullname="Not a dummy",
                                                                     password="new password", admin=False,
                                                                     activated=False), headers=header,
                                       follow_redirects=True)
