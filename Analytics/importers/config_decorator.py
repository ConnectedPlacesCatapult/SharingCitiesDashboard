    ...
    def __call__(self, cls: object):
        class Wrapped(cls):
            ...
            def get_config(self, *args: str):
                ...
                importer_status.status = Status(__name__, action="get_config", status="Get Import Configutrations")
                self.config = None
                ...
                try:
                    ...
                    importer_status.status = Status(__name__, action="Exception",
                                                    status="No Config Filer",
                                                    importer="Kill",
                                                    exception=e.__str__())
                    exit()
                ...
                if len(args) == 2:
                    try:
                        ...
                        importer_status.status = Status(__name__, action="Exception",
                                                        status="Exception fetching config keys", exception=e.__str__())
                        return None
                else:
                    self.API_NAME = None
                    self.BASE_URL = None
