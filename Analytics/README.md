...
### Data Cleaning
Upon receiving a request for forecasting, Sharing Cities Analytics API retrieves historical data for the attribute and performs an optional outlier removal using the [box-and-whiskers](https://en.wikipedia.org/wiki/Box_plot) approach.  Outliers are  measurements that are considerably different from the average, signaling erroneous measurements particularly when they occur in isolation relative to the bulk of the time series measurements. The box-and-whiskers approach computes the empirical percentiles of the time series and excludes the values that are above and below a scaled factor of 75 and 25 percentiles respectively. The scaling factor was taken to be 1.5 to be in line with common practises. 

### Temporal frequency extraction and regularisation
A time series generated by APIs is not always temporally consistent. This is due to a variety of factors, ranging from sourcing update frequency to sensor specifications. Forecasting greatly benefits from a time series with a regular temporal frequency. Thus, there is a need to regularise the series by imputing the temporal frequency window. This step estimates this frequency  by calculating the time difference between subsequent measurements and defaulting to the nearest from a set of predefined frequencies: 1 minute, 10 minutes, 60 minutes, 1 day. The time series is then resampled to the specified frequency and the resulting missing values filled with the most recent subsequent value of the series (forward filling). More missing value techniques could be implemented in the future. 

## Model optimisation
Before using the Holt-Winters model for predictions, the parameters characterising the time series characteristics have to be estimated from the data. Sharing Cities Analytics API uses the following strategy for this:

- Optimisation of Trend, Seasonality and Cycle smoothing parameters by minimising the prediction mean squared error using a 3-fold cross validation method.
- Optimisation of season length by choosing the length that minimises the fitting mean squared error

Cross Validation is a technique that partitions the data into training and test subsets. An iteration over the folds is performed training the model over the training subset and forecasting over the test subset. The mean squared error is then computed and minimised over the smoothing parameters. 

Since the time series season length is unknown beforehand,  an array of different season length values is used to fit the model over the whole dataset. The mean squared error over the fitted model is then calculated, and the length producing the best fit is used for predictions.

## Prophet API
Sharing Cities API also supports the predictive functions of [prophet](https://facebook.github.io/prophet/docs/quick_start.html). Prophet's forecasting engine uses the Holt-Winters model as well, however, it provides more parameters that allow further fine tuning of the smoothing parameters. 
